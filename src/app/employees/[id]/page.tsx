      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {employee.assignments.map((a) => (
            <Link key={a.id} href={`/projects/${a.project.id}`} className="text-xs px-2.5 py-1 rounded-full border border-[var(--border)] hover:bg-[var(--overlay-5)]">
              {a.project.name}
            </Link>
          ))}
      ...
      {leaveBalance.length > 0 && (
        <Card className="p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
            {leaveBalance.map((b) => (
              <div key={b.policyId} className="rounded-md bg-[var(--overlay-5)] px-3 py-2">
