...
export function CompanyForm({ reps }: { reps: Candidate[] }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="border border-[var(--border)] rounded-xl bg-[var(--surface)] p-5 mb-4 grid sm:grid-cols-2 gap-3">
      <input name="name" required placeholder="Company name" className="rounded-md border border-[var(--border)] bg-transparent px-3 py-2 text-sm outline-none focus:border-white/60" />
      ...
      <input name="leadSource" placeholder="Lead source" className="rounded-md border border-[var(--border)] bg-transparent px-3 py-2 text-sm outline-none focus:border-white/60" />
      <input name="estimatedValue" type="number" min={0} placeholder="Estimated deal value (₹)" className="rounded-md border border-[var(--border)] bg-transparent px-3 py-2 text-sm outline-none focus:border-white/60" />
      {reps.length > 0 && (
