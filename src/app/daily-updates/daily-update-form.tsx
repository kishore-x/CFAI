  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div>
        ...
        <textarea name="completed" defaultValue={initial?.completed ?? ""} rows={2} className="w-full rounded-md border border-[var(--border)] bg-transparent px-3 py-2 text-sm outline-none focus:border-[var(--border-60)]" />
      </div>
      <div>
        <label className="block text-xs text-[var(--muted)] mb-1">Working on</label>
        <textarea name="inProgress" defaultValue={initial?.inProgress ?? ""} rows={2} className="w-full rounded-md border border-[var(--border)] bg-transparent px-3 py-2 text-sm outline-none focus:border-[var(--border-60)]" />
      </div>
      <div>
        <label className="block text-xs text-[var(--muted)] mb-1">Blocked by</label>
        <textarea name="blocked" defaultValue={initial?.blocked ?? ""} rows={2} className="w-full rounded-md border border-[var(--border)] bg-transparent px-3 py-2 text-sm outline-none focus:border-[var(--border-60)]" />
      </div>
      <div>
        <label className="block text-xs text-[var(--muted)] mb-1">Tomorrow&apos;s plan</label>
        <textarea name="tomorrow" defaultValue={initial?.tomorrow ?? ""} rows={2} className="w-full rounded-md border border-[var(--border)] bg-transparent px-3 py-2 text-sm outline-none focus:border-[var(--border-60)]" />
      </div>
