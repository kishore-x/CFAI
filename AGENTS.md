...
- Sales CRM (`src/app/sales/`): a separate workspace for the `SALES_REP` role plus `OWNER`. Every record (Lead/Company/Opportunity/FollowUp/Meeting/Proposal) carries its own `assignedToId` — the owning rep. All CRUD lives in `src/app/sales/sales-actions.ts` and every action calls `assertSalesCRMAccess`/`assertCanAccessSalesRecord` from `authorize.ts` (never trust a client-sent id); every page re-checks ownership server-side too (`canAccessSalesRecord`) rather than relying on the list queries already being scoped. `MANAGER` and `DEVELOPER` get no access at all — this is intentional per product spec, not an oversight. Reusable pipeline/target math lives in `src/lib/sales-service.ts`.
